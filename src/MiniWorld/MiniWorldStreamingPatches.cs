...
internal static class MiniWorldStreamingPatches {
    private static readonly float MeshVertexScale = 0.25f * (
        1 << MiniWorldMeshBuilding.levelSettings.downsamples
    );
    ...
    private static IEnumerator RebuildHologram_WithStreamingAsync(MiniWorld __instance) {
        ...
        while (__instance != null) {
            if (
                ...
                (isPickupable && __instance.GetComponentInParent<Player>() == null)
            ) {
                ...
                foreach (Int3 chunkId in batches) {
                    ...
                    if ((startedLoadingPos - __instance.transform.position).sqrMagnitude > 50) {
                        continue;
                    }

                    if (__instance.GetChunkExists(chunkId)) continue;
                    AsyncOperationHandle<Mesh> request =
                        StreamMiniWorldChunkOperation.Start(chunkId);
                    ...
                    if (
                        ...
                        __instance.GetChunkExists(chunkId)
                    ) continue;
    ...
    private static void UpdateIndividualChunkPosition(
        Vector3 miniWorldStreamingOriginOffset, MiniWorld __instance,
        Int3 chunkId, MiniWorld.Chunk chunk
    ) {
        Vector3 cellPosLocalSpace =
            (chunkId * MiniWorldMeshBuilding.cellSize).ToVector3() - miniWorldStreamingOriginOffset;
