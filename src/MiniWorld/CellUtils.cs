...
internal static class CellUtils {
    [Pure]
    internal static HashSet<Int3> BatchesToLoadForGivenCell(Int3 cellId, int cellSize, ClipMapManager.LevelSettings settings) {
        Int3 offset = cellId * cellSize;
        Int3 minBlock = new(offset.x - (3 << settings.downsamples), offset.y - (3 << settings.downsamples), offset.z - (3 << settings.downsamples));
